...
/**
 * 使用index表过滤获取schedule表数据并补全roomID level group字段
...
function queryScheduleData(filters = {}) {
  try {
    ...
    console.log(`查询到 ${result.length} 条调度数据`);
    
    return result;
  } catch (error) {
    console.error('查询 schedule 数据失败:', error.message);
    throw error;
  }
}
...
/* // 查询所有调度数据
const allSchedules = queryScheduleData();

// 按设备ID筛选
const specificDevices = queryScheduleData({
  deviceIDList: [801000, 801001, 801002]
});

// 按楼层筛选
const specificLevels = queryScheduleData({
  levelList: [1, 2, 3]
});

// 按分组筛选
const specificGroups = queryScheduleData({
  groupList: [1, 2]
});

// 按房间ID筛选
const specificRooms = queryScheduleData({
  roomIDList: [1001, 1002, 2001]
});

// 组合筛选
const combinedFilters = queryScheduleData({
  levelList: [1, 2],
  groupList: [1, 2, 3],
