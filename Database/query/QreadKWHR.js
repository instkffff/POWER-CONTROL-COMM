...
/**
 * 使用index表过滤获取ReadKWHR表数据并补全roomID level group字段
 ...
 * @returns {Array} 查询结果
 */
function queryReadKWHRData(filters = {}) {
  try {
    // 创建数据库连接
    ...
    let sql = `
      SELECT 
        ...
        i."group"
      FROM ReadKWHR rk
      JOIN "index" i ON rk.deviceID = i.deviceID
      WHERE 1=1
    `;
    ...
    console.log(`查询到 ${result.length} 条电量数据`);
    
    return result;
  } catch (error) {
    console.error('查询 ReadKWHR 数据失败:', error.message);
    throw error;
  }
}

export { queryReadKWHRData };

/* // 查询所有电量数据
const allReadKWHRData = queryReadKWHRData();

// 按设备ID筛选
const specificDevices = queryReadKWHRData({
  deviceIDList: [801000, 801001, 801002]
});

// 按楼层筛选
const specificLevels = queryReadKWHRData({
  levelList: [1, 2, 3]
});

// 按分组筛选
const specificGroups = queryReadKWHRData({
  groupList: [1, 2]
});

// 按房间ID筛选
const specificRooms = queryReadKWHRData({
  roomIDList: [1001, 1002, 2001]
});

// 组合筛选
const combinedFilters = queryReadKWHRData({
  levelList: [1, 2],
  groupList: [1, 2, 3],
